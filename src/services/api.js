 ...
 * @param {boolean|string} options.useToken - Whether to use a token or not. If a string is passed, it will be used as the token.
 * @param {'json'|'x-www-form-urlencoded'|'form-data'} [options.contentType = 'json'] - The content type of the request.
 * @return {Promise<any>}
 */
async function Get({ endpoint, useToken = false, contentType = "json" }) {
    let token;
 ...
 * @param {boolean|string} options.useToken - If the request should use a token.
 * @param {'json'|'x-www-form-urlencoded'|'form-data'} [options.contentType = 'json'] - The content type of the request.
 * @return {Promise<any>}
 */
 ...
 * @return {Promise<any>}
 */
async function Delete({ endpoint, useToken = false, contentType = "json" }) {
    let token;
    if (typeof useToken === "string") {
        token = useToken;
    } else if (useToken === true) {
...
export { getAll, patch, post, delete_, get, Patch, Post, Get, Delete };
