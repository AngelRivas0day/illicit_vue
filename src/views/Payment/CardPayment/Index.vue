			...
			<div class="payment-form--inner">
				<form @submit="onSubmit" class="row w-100">
					<div class="col-12 mb-4">
						...
						<card-number class="stripe-element card-number" ref="cardNumber" :stripe="publicKey" :options="stripeOptions" @change="card.number = $event.complete" />
					</div>
					<div class="col-6">
						<label class="text-white mb-0">Expiraci&oacute;n</label><br />
						<card-expiry class="stripe-element card-expiry" ref="cardExpiry" :stripe="publicKey" :options="stripeOptions" @change="card.expiry = $event.complete" />
					</div>
					<div class="col-6">
						<label class="text-white mb-0">CVV</label><br />
						<card-cvc class="stripe-element card-cvc" ref="cardCvc" :stripe="publicKey" :options="stripeOptions" @change="card.cvc = $event.complete" />
					</div>
					<div class="col-12 text-right mt-5">
						<md-button class="md-dense md-raised md-primary" type="submit">Pagar</md-button>
					</div>
				</form>
			</div>
		</div>
...
export default {
	name: 'CardPayment',
	data: () => ({
		publicKey: 'pk_test_51HJkwAD1nUNZOF3ZYIn3DEBY2QSkJdQTAYMYajExWnVXVnRBpiW1zmDJy2Ee1f3hzvmRDeu0kbmN78yMUsagfy2400HkhbwZ14',
		card: {
			complete: false,
			number: false,
			expiry: false,
