				...
				<div class="navbar__container">
					<router-link to="/" class="navbar__logo">
						<img
							width="30"
							height="30"
							:src="require(whiteIcons ? '@/assets/img/v2/white_small_logo.png' : '@/assets/img/v2/small_logo.png')"
							:alt="whiteIcons ? 'Small white ILLICIT logo' : 'Small black ILLICIT logo'"
						/>
					</router-link>
					<div class="navbar__actions d-flex flex-row justify-content-center align-items-center">
						<navbar-account :white-icons="whiteIcons" :is-logged-in="isLoggedIn" />
						<navbar-favorites :white-icons="whiteIcons" :favorites="favorites" />
...
export default {
	name: "MainLayout",
	components: {SidebarOptions, NavbarFavorites, NavbarAccount, Footer, EventSnackBar},
	async mounted() {
		this.watchAuthState();
	...
	methods: {
		async fetchFavorites() {
			try {
				...
				this.$notify({
					...
					message: 'Ha ocurrido un error al obtener tus favoritos. Por favor, intenta más tarde.',
					type: 'error',
				});
			}
		},
		...
		async fetchEvents() {
			try {
				...
				this.$notify({
					...
					message: 'Ha ocurrido un error al obtener los eventos. Por favor, intenta más tarde.',
					type: 'error',
				});
			}
		},
		addEventsToMenuOptions(events) {
			events = events.map(event => ({
				id: event.id,
				label: event.name,
				routeData: {name: 'Products', params: {id: event.id}},
				children: [],
			}));
		...
		addCategoriesToMenuOptions(categories) {
			categories = categories.map(category => ({
				id: category.id,
				label: category.name,
				routeData: {name: 'Products', query: {category_id: category.id}},
				children: [],
			}));
			const option = this.menuOptions.find(option => option.label === 'categorias');
			if (!option) return;
			// ?: Refresh the ID to trigger the reactivity
			option.id = 66;
		...
		watchAuthState() {
			const auth = getAuth();
			this.unsubscribeAuth = auth.onAuthStateChanged((user) => this.isLoggedIn = !!user);
		},
